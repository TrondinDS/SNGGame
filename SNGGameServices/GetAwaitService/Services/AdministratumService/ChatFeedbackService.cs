...
public class ChatFeedbackService : IChatFeedbackService
{
    ...
    public async Task<IEnumerable<ChatFeedbackDTO>?> Filter(ParamQueryChatfeedback param)
    {
        ...
        var response = await _httpClient.PostAsync("api/ChatFeedback/Filter", content);
        if (!response.IsSuccessStatusCode) return null;

        return await response.Content.ReadFromJsonAsync<IEnumerable<ChatFeedbackDTO>>(_jsonOptions);
    }
}
